...
Meteor.methods({
	...
	upvote: function(problemId) {
		...
		if(!problem)
			throw new Meteor.Error('invalid', 'Problem not found');

		if(_.include(problem.upvoters, this.userId))
			throw new Meteor.Error('invalid', 'Already upvoted this problem');

		Problems.update(problem._id, {
